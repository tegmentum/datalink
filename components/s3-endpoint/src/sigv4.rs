...
//!
//! Reused verbatim from ducklink's `s3fs-component/src/sigv4.rs` (the only
//! change is that `sha256_hex` is now `pub` so PUT/COPY callers can hash a
//! request body). The full `aws-sigv4` crate drags in the aws-smithy async
//! runtime + http + bytes machinery; for a signed request it is far lighter
//! (and more portable to wasm32-wasip2) to compute the four SigV4 steps
//! directly on `hmac` + `sha2`, which build cleanly for wasip2:
//!
...
pub struct Credentials {
    ...
    /// Carried for API symmetry; the session token is signed as the
    /// `x-amz-security-token` header by the caller, not inside `sign_v4`.
    #[allow(dead_code)]
    pub session_token: Option<String>,
}

/// SHA-256 of `data`, hex-encoded. Used both for the canonical-request hash
/// and (for PUT/COPY) for the `x-amz-content-sha256` payload header.
...
/// Percent-encode an arbitrary string for the canonical query string (every
/// non-unreserved byte encoded, including '/').
...
/// Build the canonical query string from `(name, value)` pairs: each component
/// URI-encoded, sorted by encoded name, joined with `&`. AWS SigV4 requires
/// this exact form for the signature to match.
...
mod tests {
    ...
    /// AWS's published SigV4 example: "GET Object" with a Range header. The
    /// docs give the expected signature for this exact request, so this is an
    /// offline, deterministic proof the signing path is correct.
    #[test]
    fn aws_doc_get_object_vector() {
        ...
        let authz = sign_v4(
            &creds, "GET", "us-east-1", "s3", "/test.txt", "", &headers,
            EMPTY_PAYLOAD_SHA256, "20130524T000000Z",
        );
        assert!(
            authz.contains(
