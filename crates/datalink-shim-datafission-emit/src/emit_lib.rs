...
//!   8. Stub impls for custom-type / multi-custom-type — empty
//!      advertisements + `Internal` errors.
...
//!
//! Scalar-first cut: scalars are wired against the
//! `datafission:extension@1.0.0` composite world; aggregates,
//! window functions, table functions, custom types, spatial
//! indexes, system catalog, and 1D index plugin export stubs that
//! advertise nothing and return `Unknown*` / `Internal` errors on
//! per-call paths. See AGENTS.md (in
...
use bindings::exports::datafission::type_plugin::custom_type;
...
"##,
    ...
    fn api_version() -> String {{ "1.0.0".into() }}
}}
...
"##,
    ...
    // ---- custom-type: stub (single placeholder type) ----
    s.push_str(&format!(
        r##"impl custom_type::Guest for Component {{
    fn type_name() -> String {{ "{primary}_unspecified".into() }}
    fn type_id() -> u32 {{ 0 }}
    fn storage_size() -> i32 {{ -1 }}

    fn serialize(value: Vec<u8>) -> Vec<u8> {{ value }}

    fn deserialize(bytes: Vec<u8>) -> Result<Vec<u8>, ttypes::TypeError> {{
        Ok(bytes)
    }}

    fn compare(a: Vec<u8>, b: Vec<u8>) -> ttypes::Ordering {{
        match a.cmp(&b) {{
            core::cmp::Ordering::Less => ttypes::Ordering::Less,
            core::cmp::Ordering::Equal => ttypes::Ordering::Equal,
            core::cmp::Ordering::Greater => ttypes::Ordering::Greater,
        }}
    }}

    fn hash_value(value: Vec<u8>) -> u64 {{
    ...
    fn display(value: Vec<u8>) -> String {{
        format!("<{primary}:{{}}b>", value.len())
    }}

    fn parse(input: String) -> Result<Vec<u8>, ttypes::TypeError> {{
        Ok(input.into_bytes())
    }}

    fn cast_from() -> Vec<ttypes::LogicalType> {{ Vec::new() }}
    fn cast_to() -> Vec<ttypes::LogicalType> {{ Vec::new() }}
}}
