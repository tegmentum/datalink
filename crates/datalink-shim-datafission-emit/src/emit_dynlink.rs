...
/// Dynlink-mode datafission bridge — Phase 3 Commit 6 (#823).
///
...
/// The datafission composite export set is unchanged.
world bridge {{
    // Compose:dynlink linker — the only shim-side import.
    import compose:dynlink/linker@0.1.0;

    // Datafission composite exports (mirrors the WacPlug emit).
...
fn lib_rs(plan: &BridgePlan, opts: &DynlinkOptions) -> Result<String> {
    ...
    for (entry, fallible) in &primitive_arms {
        ...
        metas_block.push_str(&format!(
            "        ftypes::ScalarFunctionMeta {{ name: \"{escaped}\".to_string(), aliases: alloc::vec![], param_types: alloc::vec![{sig}], is_deterministic: true, propagates_null: true }},\n",
        ));
...
//! (Phase 3 Commit 6, #823). Do NOT edit by hand — regenerate.
#![no_std]
extern crate alloc;
...
use datafission::extension::identity;
use datafission::function_plugin::{{
...
use datafission::sql_extension_plugin::metadata as se_meta;
use datafission::sql_extension_plugin::types as setypes;
use datafission::type_plugin::{{custom_type, multi_custom_type, types as ttypes}};
use datafission::spatial_index_plugin::{{spatial_index, types as sitypes}};
use datafission::system_catalog_plugin::system_catalog;
use datafission::index_plugin::{{index, types as itypes}};

use compose::dynlink::linker;

const PROVIDER_ID: &str = "{provider_id}";

/// Cached instance handle. Resolved on first use.
static INSTANCE: core::sync::atomic::AtomicU32 = core::sync::atomic::AtomicU32::new(0);

fn resolve() -> Result<linker::Instance, ftypes::FunctionError> {{
    // wit-bindgen materialises `resolve-by-id` as a free function on
    // the `linker` module. The returned Instance owns its handle.
    linker::resolve_by_id(&PROVIDER_ID.to_string())
        .map_err(|e| ftypes::FunctionError::ExecutionError(alloc::format!("dynlink resolve('{{}}'): {{:?}}", PROVIDER_ID, e)))
}}
...
impl se_meta::Guest for Component {{
    ...
    fn source_type_id() -> String {{ "{primary}".to_string() }}
}}
...
// Empty registries for the other capabilities.
impl aggregate_function_registry::Guest for Component {{
    fn list_functions() -> Vec<ftypes::AggregateFunctionMeta> {{ Vec::new() }}
    fn return_type(_name: String, _i: Vec<ftypes::LogicalType>) -> Result<ftypes::LogicalType, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no aggregates in dynlink bridge".to_string())) }}
    fn create_accumulator(_name: String) -> Result<u64, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no aggregates".to_string())) }}
    fn create_accumulator_with_config(_name: String, _cfg: Vec<u8>) -> Result<u64, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no aggregates".to_string())) }}
    fn create_accumulator_with_configs(_name: String, _cfgs: Vec<Vec<u8>>) -> Result<u64, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no aggregates".to_string())) }}
    fn accumulate(_h: u64, _args: Vec<ftypes::ScalarValue>) -> Result<(), ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no aggregates".to_string())) }}
    fn merge(_h: u64, _o: u64) -> Result<(), ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no aggregates".to_string())) }}
    fn finalize(_h: u64) -> Result<ftypes::ScalarValue, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no aggregates".to_string())) }}
    fn destroy(_h: u64) {{}}
}}

impl window_function_registry::Guest for Component {{
    fn list_functions() -> Vec<ftypes::WindowFunctionMeta> {{ Vec::new() }}
    fn return_type(_name: String, _i: Vec<ftypes::LogicalType>) -> Result<ftypes::LogicalType, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no windows".to_string())) }}
    fn compute_partition(_name: String, _rows: Vec<Vec<ftypes::ScalarValue>>) -> Result<Vec<ftypes::ScalarValue>, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no windows".to_string())) }}
}}

impl table_function_registry::Guest for Component {{
    fn list_functions() -> Vec<ftypes::TableFunctionMeta> {{ Vec::new() }}
    fn output_schema(_name: String, _i: Vec<ftypes::LogicalType>) -> Result<Vec<ftypes::ColumnInfo>, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no tables".to_string())) }}
    fn begin(_name: String, _args: Vec<ftypes::ScalarValue>) -> Result<u64, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no tables".to_string())) }}
    fn next_row(_h: u64) -> Result<Option<Vec<ftypes::ScalarValue>>, ftypes::FunctionError> {{ Err(ftypes::FunctionError::UnknownFunction("no tables".to_string())) }}
    fn end(_h: u64) {{}}
}}

impl custom_type::Guest for Component {{
    fn list_types() -> Vec<ttypes::CustomTypeMeta> {{ Vec::new() }}
    fn parse(_name: String, _text: String) -> Result<Vec<u8>, ttypes::TypeError> {{ Err(ttypes::TypeError::Internal("no custom types".to_string())) }}
    fn format(_name: String, _bytes: Vec<u8>) -> Result<String, ttypes::TypeError> {{ Err(ttypes::TypeError::Internal("no custom types".to_string())) }}
}}

impl multi_custom_type::Guest for Component {{
    fn list_types() -> Vec<ttypes::CustomTypeMeta> {{ Vec::new() }}
    fn parse(_name: String, _text: String) -> Result<Vec<u8>, ttypes::TypeError> {{ Err(ttypes::TypeError::Internal("no custom types".to_string())) }}
    fn format(_name: String, _bytes: Vec<u8>) -> Result<String, ttypes::TypeError> {{ Err(ttypes::TypeError::Internal("no custom types".to_string())) }}
}}

impl spatial_index::Guest for Component {{
    fn name() -> String {{ "{primary}-stub-spatial".to_string() }}
    fn aliases() -> Vec<String> {{ Vec::new() }}
    fn capabilities() -> sitypes::IndexCapabilities {{
        sitypes::IndexCapabilities {{
            supports_envelope_query: false,
            supports_knn: false,
            supports_within_distance: false,
            supports_within_distance_wkb: false,
        }}
    }}
    fn build(_items: Vec<sitypes::BuildItem>) -> Result<u64, sitypes::SpatialError> {{
        Err(sitypes::SpatialError::UnsupportedOperation("dynlink bridge stub".to_string()))
    }}
    fn entry_count(_h: u64) -> u64 {{ 0 }}
    fn query_envelope(_h: u64, _e: sitypes::Envelope) -> Result<Vec<u64>, sitypes::SpatialError> {{ Err(sitypes::SpatialError::UnsupportedOperation("stub".to_string())) }}
    fn query_knn(_h: u64, _q: Vec<u8>, _k: u32) -> Result<Vec<u64>, sitypes::SpatialError> {{ Err(sitypes::SpatialError::UnsupportedOperation("stub".to_string())) }}
    fn query_within_distance(_h: u64, _q: sitypes::Envelope, _d: f64) -> Result<Vec<u64>, sitypes::SpatialError> {{ Err(sitypes::SpatialError::UnsupportedOperation("stub".to_string())) }}
    fn query_within_distance_wkb(_h: u64, _q: Vec<u8>, _d: f64) -> Result<Vec<u64>, sitypes::SpatialError> {{ Err(sitypes::SpatialError::UnsupportedOperation("stub".to_string())) }}
    fn destroy(_h: u64) {{}}
}}

impl system_catalog::Guest for Component {{
    fn catalog_name() -> String {{ "{primary}".to_string() }}
    fn list_tables() -> Vec<system_catalog::TableInfo> {{ Vec::new() }}
}}

impl index::Guest for Component {{
    fn name() -> String {{ "{primary}-stub-index".to_string() }}
    fn type_id() -> u32 {{ 0 }}
    fn aliases() -> Vec<String> {{ Vec::new() }}
    fn capabilities() -> itypes::IndexCapabilities {{
        itypes::IndexCapabilities {{
    ...
    fn supported_types() -> Vec<ftypes::LogicalType> {{ Vec::new() }}
    fn build(_i: Vec<itypes::BuildItem>) -> Result<u64, itypes::IndexError> {{ Err(itypes::IndexError::Internal(alloc::format!("{primary} index-plugin: build stub"))) }}
    fn insert(_h: u64, _key: Vec<u8>, _row_id: u64) -> Result<(), itypes::IndexError> {{ Err(itypes::IndexError::Internal("insert stub".to_string())) }}
    fn delete(_h: u64, _key: Vec<u8>, _row_id: u64) -> Result<(), itypes::IndexError> {{ Err(itypes::IndexError::Internal("delete stub".to_string())) }}
    fn lookup(_h: u64, _key: Vec<u8>) -> Result<Vec<u64>, itypes::IndexError> {{ Err(itypes::IndexError::Internal("lookup stub".to_string())) }}
    fn range_scan(_h: u64, _lo: Option<Vec<u8>>, _hi: Option<Vec<u8>>) -> Result<Vec<(Vec<u8>, u64)>, itypes::IndexError> {{ Err(itypes::IndexError::Internal("range stub".to_string())) }}
    fn destroy(_h: u64) {{}}
}}
