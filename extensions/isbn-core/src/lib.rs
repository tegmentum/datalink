...
pub mod logic {
    ...
    pub fn is_valid(body: &str) -> bool {
        match body.len() {
            10 => valid_isbn10(body),
            13 => valid_isbn13(body),
            _ => false,
        }
    }
}
...
mod tests {
    ...
    fn matches_baseline() {
        assert_eq!(
            Core::dispatch(idx("isbn_valid"), &[t("0-306-40615-2")]).unwrap(),
            NeutralValue::Boolean(true)
        );
        assert_eq!(
            Core::dispatch(idx("isbn_valid"), &[t("978-0-306-40615-7")]).unwrap(),
            NeutralValue::Boolean(true)
        );
        assert_eq!(
            Core::dispatch(idx("isbn_valid"), &[t("0-306-40615-3")]).unwrap(),
            NeutralValue::Boolean(false)
        );
        assert_eq!(
            Core::dispatch(idx("isbn_normalize"), &[t("0 306 40615 2")]).unwrap(),
            t("0306406152")
        );
        assert_eq!(
            Core::dispatch(idx("isbn_normalize"), &[t("not-an-isbn")]).unwrap(),
            NeutralValue::Null
        );
    }
}
