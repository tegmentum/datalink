...
## Parser extensions (the `ggsql-core` pilot)

`ggsql-core` is the first PARSER pull-up. A parser extension intercepts
custom statement syntax that desugars to ordinary SQL (e.g.
`VISUALIZE <select>` -> a grouped `(label, n, bar)` rollup): text in, SQL
text out — no bound parse tree crosses the WIT boundary.

The DB-neutral engine is `parse_visualize(&str, &Dialect) -> Outcome`.
The *parsing* (keyword recognition, inner-select extraction, the
never-panic contract) is shared verbatim; only the emitted rewrite's
DIALECT differs (DuckDB has `repeat`/`GREATEST`/`VARCHAR`/`BIGINT`,
SQLite has none of those), so each shim passes a ~3-line `Dialect` table
(`DUCKDB` / `SQLITE`).

How each DB offers a failed statement to the extension differs, so the
two shims adapt the same `Outcome`:

- **ducklink** has a first-class `ParserExtension` hook
  (`parser-dispatch.call-parse`). The shim is a ~12-line hand-written
  adapter mapping `Outcome` -> `parse-outcome` / `duckerror`.
- **sqlink** — SQLite's amalgamation parser is NOT extensible, so there
  is no in-engine hook. The host shell INTERCEPTS statements the
  built-in parser rejects (`extension-loader.dispatch-parse` ->
  `Host::dispatch_parse`) and offers them to a reserved entrypoint
  scalar `__sqlink_parse` (`ggsql_core::PARSE_FN`). Because the
  entrypoint is an ordinary scalar, the **sqlink parser shim is fully
  write-once-generated by `sqlite_shim!`** — no bespoke parser glue, no
  new host world/bindgen.

Codegen status (honest): the sqlink side already rides the scalar
codegen. The remaining follow-on is a `parser!` `declare!` arm that ALSO
emits the ducklink parser-dispatch adapter from one declaration (today
that adapter is the only hand-written piece). Proven end-to-end in BOTH
DBs: `VISUALIZE SELECT 'apple' AS label, 3 AS n UNION ALL SELECT 'pear',
1` -> `(apple, 3, ###)` / `(pear, 1, #)` (sqlink: `host/tests/parser.rs`;
ducklink: native DuckDB on the `DUCKDB`-dialect rewrite).
